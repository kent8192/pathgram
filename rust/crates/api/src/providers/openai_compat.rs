...
impl OpenAiCompatConfig {
    ...
    /// `DeepSeek`'s OpenAI-compatible endpoint. The public pricing page lists
    /// deepseek-v4-flash/pro as materially cheaper than Claude, xAI, and Qwen
    /// for normal token mixes, so keep it as a first-class route instead of
    /// forcing users through `OPENAI_BASE_URL`.
    #[must_use]
...
fn strip_routing_prefix(model: &str) -> &str {
    if let Some(pos) = model.find('/') {
        ...
        if matches!(
            prefix,
            "openai" | "xai" | "grok" | "qwen" | "kimi" | "deepseek" | "local"
        ) {
            &model[pos + 1..]
        } else {
            model
