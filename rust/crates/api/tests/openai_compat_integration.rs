...
async fn provider_client_dispatches_deepseek_requests_from_env() {
    ...
    let server = spawn_server(
        ...
        vec![http_response(
            "200 OK",
            "application/json",
            "{\"id\":\"chatcmpl_deepseek\",\"model\":\"deepseek-v4-flash\",\"choices\":[{\"message\":{\"role\":\"assistant\",\"content\":\"Through DeepSeek\",\"tool_calls\":[]},\"finish_reason\":\"stop\"}],\"usage\":{\"prompt_tokens\":7,\"completion_tokens\":3}}",
        )],
    )
    .await;
    let _base_url = ScopedEnvVar::set("DEEPSEEK_BASE_URL", server.base_url());

    let client = ProviderClient::from_model("deepseek-flash")
