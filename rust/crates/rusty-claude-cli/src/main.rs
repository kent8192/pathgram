...
fn validate_model_syntax(model: &str) -> Result<(), String> {
    ...
    if parts.len() != 2 || parts[0].is_empty() || parts[1].is_empty() {
        ...
        let mut err_msg = format!(
            "invalid model syntax: '{}'.\nExpected provider/model (e.g., anthropic/claude-opus-4-7) or known alias (opus, sonnet, haiku, deepseek)",
            trimmed
        );
            ...
            println!(
                ...
                serde_json::to_string_pretty(&json!({
                    ...
                    "aliases": [
                        ...
                        {"name": "deepseek", "model": resolve_model_alias("deepseek")},
                        {"name": "deepseek-flash", "model": resolve_model_alias("deepseek-flash")},
                        {"name": "deepseek-pro", "model": resolve_model_alias("deepseek-pro")}
                    ],
...
mod tests {
    ...
    #[test]
    fn default_model_uses_deepseek_flash() {
        assert_eq!(DEFAULT_MODEL, "deepseek-v4-flash");
        assert_eq!(resolve_model_alias("opus"), "anthropic/claude-opus-4-7");
        assert_eq!(resolve_model_alias("deepseek"), DEFAULT_MODEL);
    }
...
mod alias_resolution_tests {
    ...
    fn test_alias_resolution_builtin() {
        ...
        assert_eq!(
            resolve_model_alias_with_config("deepseek"),
            "deepseek-v4-flash"
        );
        assert_eq!(
            resolve_model_alias_with_config("deepseek-pro"),
            "deepseek-v4-pro"
        );
    }
    ...
    fn test_alias_resolution_syntax_validation() {
        ...
        // Raw built-in aliases are accepted for CLI ergonomics.
        assert!(validate_model_syntax("opus").is_ok());
